...
import static scratch.newast.parser.stmt.EntityLookStmtParser.INPUTS;
...
public class ProcDeclParser {
    ...
    private static ProcedureDeclaration parseProcDecl(JsonNode def, JsonNode blocks){
        JsonNode input = def.get(INPUTS).get(CUSTOM_BLOCK);
        ...
        Iterator<Map.Entry<String, JsonNode>> iter = proto.get(INPUTS).fields();
