...
final class LedMatrixStmtConverter extends StmtConverter<LEDMatrixStmt> {
    ...
    LEDMatrixStmt convertStmt(final RawBlockId blockId, final RawBlock.RawRegularBlock block) {
        ...
        return switch (opcode) {
            ...
            case show_led_matrix_face_position -> {
                final LEDMatrix matrix = getPanel(block);
                // note the different keys here AXIS_X instead of X_AXIS_KEY
                final NumExpr x = NumExprConverter.convertNumExpr(state, block, KnownInputs.AXIS_X);
                final NumExpr y = NumExprConverter.convertNumExpr(state, block, KnownInputs.AXIS_Y);
            ...
            case show_text -> {
                final MCorePort port = getPort(block);
                final StringExpr text = StringExprConverter.convertStringExpr(state, block, KnownInputs.TEXT);
            ...
            case show_number -> {
                final MCorePort port = getPort(block);
                final NumExpr number = NumExprConverter.convertNumExpr(state, block, KnownInputs.NUMBER);
                yield new LEDNumPort(port, number, metadata);
            }
            case show_time -> {
                final MCorePort port = getPort(block);
                final NumExpr hour = NumExprConverter.convertNumExpr(state, block, KnownInputs.NUMBER1);
                final NumExpr minute = NumExprConverter.convertNumExpr(state, block, KnownInputs.NUMBER2);
    ...
    private NumExpr getX(final RawBlock.RawRegularBlock block) {
        return NumExprConverter.convertNumExpr(state, block, KnownInputs.X_AXIS);
    }

    private NumExpr getY(final RawBlock.RawRegularBlock block) {
        return NumExprConverter.convertNumExpr(state, block, KnownInputs.Y_AXIS);
    }
}
