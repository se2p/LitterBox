...
public class LoudnessOutOfBounds extends AbstractRobotFinder {
    private static final String NAME = "loudness_out_of_bounds";
    ...
    public void visit(Equals node) {
        ...
        if (setValue && hasLoudness) {
            if (sensorValue > LOUDNESS_MAX || sensorValue < LOUDNESS_MIN) {
    ...
    public void visit(BiggerThan node) {
        visitComp(node);
        if (setValue && hasLoudness) {
            if ((firstHasLoudness && sensorValue >= LOUDNESS_MAX) || (secondHasLoudness && sensorValue <= LOUDNESS_MIN)) {
                addIssue(node, IssueSeverity.LOW);
            }
        }
        insideComparison = false;
    ...
    public void visit(LessThan node) {
        visitComp(node);
        if (setValue && hasLoudness) {
            if ((firstHasLoudness && sensorValue <= LOUDNESS_MIN) || (secondHasLoudness && sensorValue >= LOUDNESS_MAX)) {
                addIssue(node, IssueSeverity.LOW);
            }
        }
        insideComparison = false;
