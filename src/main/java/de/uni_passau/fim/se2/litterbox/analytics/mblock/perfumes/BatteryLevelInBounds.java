...
public class BatteryLevelInBounds extends AbstractRobotFinder {
    private static final String NAME = "battery_level_in_bounds";
    ...
    public void visit(BiggerThan node) {
        ...
        if (setValue && hasBatteryLevel) {
            if (((firstHasBattery && sensorValue >= BATTERY_LEVEL_MIN && sensorValue < BATTERY_LEVEL_MAX)
                    || (secondHasBattery && sensorValue <= BATTERY_LEVEL_MAX && sensorValue > BATTERY_LEVEL_MIN)) && (sensorValue % 10) == 0) {
    ...
    public void visit(LessThan node) {
        visitComp(node);
        if (setValue && hasBatteryLevel) {
            if (((firstHasBattery && sensorValue <= BATTERY_LEVEL_MAX && sensorValue > BATTERY_LEVEL_MIN)
                    || (secondHasBattery && sensorValue >= BATTERY_LEVEL_MIN && sensorValue < BATTERY_LEVEL_MAX)) && (sensorValue % 10) == 0) {
                addIssue(node, IssueSeverity.LOW);
            }
        }
        insideComparison = false;
