...
public class ActorJSONCreator {
    ...
    public static String createActorJSONString(ActorDefinition actor) {
        ...
        for (int i = 0; i < broadcasts.size() - 1; i++) {
            ...
            JSONStringCreator.createFieldValue(jsonString, current.getBroadcastID(), current.getBroadcastName()).append(",");
        }
        ...
        for (int i = 0; i < procedures.size() - 1; i++) {
            jsonString.append(ProcedureJSONCreator.createProcedureJSONString(procedures.get(i))).append(",");
        }
        ...
        for (int i = 0; i < scripts.size() - 1; i++) {
            jsonString.append(ScriptJSONCreator.createScriptJSONString(scripts.get(i))).append(",");
        }
        if (scripts.size() > 0) {
            jsonString.append(ScriptJSONCreator.createScriptJSONString(scripts.get(scripts.size() - 1)));
        }
        jsonString.append("},");
