...
public class PitchAngleOutOfBounds extends AbstractRobotFinder {
    private static final String NAME = "pitch_angle_out_of_bounds";
    ...
    public void visit(Equals node) {
        ...
        if (setValue && hasPitchAngle) {
            if (sensorValue > PITCH_ANGLE_MAX || sensorValue < PITCH_ANGLE_MIN) {
    ...
    public void visit(BiggerThan node) {
        visitComp(node);
        if (setValue && hasPitchAngle) {
            if ((firstHasPitchAngle && sensorValue >= PITCH_ANGLE_MAX) || (secondHasPitchAngle && sensorValue <= PITCH_ANGLE_MIN)) {
                addIssue(node, IssueSeverity.LOW);
            } else if ((sensorValue % 1) != 0) {
                addIssue(node, IssueSeverity.LOW);
            }
    ...
    public void visit(LessThan node) {
        visitComp(node);
        if (setValue && hasPitchAngle) {
            if ((firstHasPitchAngle && sensorValue <= PITCH_ANGLE_MIN) || (secondHasPitchAngle && sensorValue >= PITCH_ANGLE_MAX)) {
                addIssue(node, IssueSeverity.LOW);
            } else if ((sensorValue % 1) != 0) {
                addIssue(node, IssueSeverity.LOW);
            }
