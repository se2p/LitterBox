    ...
    public void visit(RepeatTimesStmt node) {
        if (node.getTimes() instanceof NumberLiteral) {
            if (((NumberLiteral) node.getTimes()).getValue() == 1) {
                IssueBuilder builder = prepareIssueBuilder().withSeverity(IssueSeverity.LOW).withCurrentNode(node).withMetadata(node.getMetadata()).withHint(new Hint(ONE_HINT));
                ...
                IssueBuilder builder = prepareIssueBuilder().withSeverity(IssueSeverity.LOW).withCurrentNode(node).withMetadata(node.getMetadata()).withHint(new Hint(ZERO_HINT));
